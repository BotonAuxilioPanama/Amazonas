...
cabecera "Tabla Por Modalidad", paginaTit
...
<section class="col-md-12 col-xs-12">
	<h2>Fecha: <%=modalidades(1,g)%></h2>
		<%
					...
					RsFechas.Source = "SELECT Recorridos.Fecha FROM (Competencia INNER JOIN Equipos ON Competencia.idCompetencia = Equipos.idCompetencia) INNER JOIN (Amazonas INNER JOIN (Modalidad INNER JOIN Recorridos ON Modalidad.idModalidad = Recorridos.idModalidad) ON Amazonas.idAmazona = Recorridos.idAmazona) ON Equipos.idEquipo = Amazonas.idEquipo  WHERE (((Modalidad.idModalidad)="&idMod&")) GROUP BY Recorridos.Fecha, Recorridos.Fecha, Competencia.idCompetencia HAVING (((Competencia.idCompetencia)="&comp&")) ORDER BY Recorridos.Fecha;"
					RsFechas.Open, Con
